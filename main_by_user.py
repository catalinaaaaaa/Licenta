...
for hyperparameters in hyperparameters_list:
    ...
    for i, model in enumerate(list_of_models):
        ...
        with open(
            f"./saves/by_user/model_{function}_{n_hidden_layers}_{i+1}.json", "w"
        ) as json_file:
            json_file.write(model_json)
        model.save_weights(
            f"./saves/by_user/model_{function}_{n_hidden_layers}_{i+1}.h5"
        )
...
with open("./saves/by_user/best_model.json", "w") as json_file:
    json_file.write(model_json)
best_model.save_weights("./saves/by_user/best_model.h5")
